...
class GrowthStore:
    ...
    def _create_tables(self) -> None:
        ...
        try:
            ...
            ''')
            proposal_cols = {row[1] for row in c.execute("PRAGMA table_info(phase7_growth_proposals)").fetchall()}
            if "evidence_receipt_id" not in proposal_cols:
                c.execute("ALTER TABLE phase7_growth_proposals ADD COLUMN evidence_receipt_id TEXT")
            ...
            ''')
            c.execute('''
            CREATE TABLE IF NOT EXISTS phase7_scale_step_proofs (
                scale_step_proof_id TEXT PRIMARY KEY,
                created_ts REAL,
                stage_id INTEGER,
                evidence_receipt_id TEXT,
                evidence_hash TEXT,
                completed_round_trips INTEGER,
                new_round_trips INTEGER,
                status TEXT,
                consumed_ts REAL,
                consumed_by_phase6_approval_id TEXT,
                payload TEXT
            )
            ''')
            c.execute(
                "CREATE INDEX IF NOT EXISTS idx_phase7_scale_step_proofs_stage "
                "ON phase7_scale_step_proofs(stage_id, created_ts, status)"
            )
            self.conn.commit()
        finally:
            if lock:
        ...
        with self._lock:
            cur = self.conn.execute(
                ...
                (proposal_id,from_stage,to_stage,proposed_by,created_ts,cooldown_until_ts,expires_ts,
                 evidence_hash,evidence_receipt_id,config_hash,evidence,reasons,status,payload)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    ...
                    proposal.get("expires_ts"), proposal.get("evidence_hash"), proposal.get("evidence_receipt_id"),
    ...
    def persist_scale_step_proof(self, proof: Mapping[str, Any]) -> bool:
        with self._lock:
            cur = self.conn.execute(
                """INSERT OR IGNORE INTO phase7_scale_step_proofs
                (scale_step_proof_id,created_ts,stage_id,evidence_receipt_id,evidence_hash,
                 completed_round_trips,new_round_trips,status,payload)
                VALUES (?,?,?,?,?,?,?,?,?)""",
                (
                    proof.get("scale_step_proof_id"),
                    proof.get("created_ts"),
                    proof.get("stage_id"),
                    proof.get("evidence_receipt_id"),
                    proof.get("evidence_hash"),
                    proof.get("completed_round_trips"),
                    proof.get("new_round_trips"),
                    proof.get("status", "MINTED"),
                    json.dumps(dict(proof), sort_keys=True, default=str),
                ),
            )
            self.conn.commit()
            return bool(cur.rowcount)

    def latest_scale_step_proof(self, *, stage_id: Optional[int] = None) -> dict[str, Any]:
        if stage_id is None:
            return self._fetchone(
                "SELECT * FROM phase7_scale_step_proofs ORDER BY created_ts DESC LIMIT 1"
            )
        return self._fetchone(
            "SELECT * FROM phase7_scale_step_proofs WHERE stage_id=? ORDER BY created_ts DESC LIMIT 1",
            (int(stage_id),),
        )

    def consume_scale_step_proof(
        self,
        scale_step_proof_id: str,
        *,
        phase6_approval_id: str,
        now_ts: Optional[float] = None,
    ) -> bool:
        now = float(now_ts or time.time())
        with self._lock:
            cur = self.conn.execute(
                """UPDATE phase7_scale_step_proofs
                SET status='CONSUMED', consumed_ts=?, consumed_by_phase6_approval_id=?
                WHERE scale_step_proof_id=? AND status='MINTED'""",
                (now, str(phase6_approval_id or ""), str(scale_step_proof_id or "")),
            )
            self.conn.commit()
            return bool(cur.rowcount)

    def scale_step_proofs(self, limit: int = 50) -> list[dict[str, Any]]:
        return self._fetchall(
            "SELECT * FROM phase7_scale_step_proofs ORDER BY created_ts DESC LIMIT ?",
            (int(limit),),
        )
