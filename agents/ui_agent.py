...
<body>
  ...
  <main>
    ...
    <section class="span-8">
      ...
      <form id="planForm">
        ...
        <label><div class="label">Symbol</div><input name="symbol" placeholder="PEPE/USDT"></label>
  ...
  <script>
    ...
    loadPlane().catch(err => { document.getElementById('rawOut').textContent = String(err); });
        ...
        def shadow_flight_json():
            try:
                ...
                if self.coordinator and hasattr(self.coordinator, "shadow_flight_snapshot"):
                    return jsonify(self.coordinator.shadow_flight_snapshot(limit=limit, compact=compact))
                return jsonify({
        ...
        @self.app.route("/commons.json")
        def commons_json():
            try:
                limit = int(request.args.get("limit", 100) or 100)
                if self.coordinator and hasattr(self.coordinator, "commons_snapshot"):
                    return jsonify(self.coordinator.commons_snapshot(limit=limit))
                return jsonify({
                    "date": "2026-08-05",
                    "commons_pool_enabled": False,
                    "authority": {"execution_authority": "none", "promotion_authority": "none"},
                    "error": "coordinator_unavailable",
                })
            except Exception as e:
                logging.exception("commons.json error")
                return jsonify({"date": "2026-08-05", "commons_pool_enabled": False, "error": str(e)}), 500

        @self.app.route("/commons/inference.json")
        def commons_inference_json():
            try:
                limit = int(request.args.get("limit", 100) or 100)
                if self.coordinator and hasattr(self.coordinator, "commons_snapshot"):
                    snapshot = self.coordinator.commons_snapshot(limit=limit)
                    return jsonify({
                        "date": "2026-08-05",
                        "authority": snapshot.get("authority") or {},
                        "tickets": [row for row in (snapshot.get("tickets") or []) if row.get("pool_type") == "inference_pool"],
                        "inference_receipts": snapshot.get("inference_receipts") or [],
                        "local_adoption_receipts": [
                            row for row in (snapshot.get("local_adoption_receipts") or [])
                            if str(row.get("pool_type") or "") == "inference_pool"
                        ],
                        "phase2": snapshot.get("phase2") or {},
                        "phase3": snapshot.get("phase3") or {},
                    })
                return jsonify({"date": "2026-08-05", "error": "coordinator_unavailable"}), 503
            except Exception as e:
                logging.exception("commons/inference.json error")
                return jsonify({"date": "2026-08-05", "error": str(e)}), 500

        @self.app.route("/commons/verifier.json")
        def commons_verifier_json():
            try:
                limit = int(request.args.get("limit", 100) or 100)
                if self.coordinator and hasattr(self.coordinator, "commons_snapshot"):
                    snapshot = self.coordinator.commons_snapshot(limit=limit)
                    return jsonify({
                        "date": "2026-08-05",
                        "authority": snapshot.get("authority") or {},
                        "tickets": [row for row in (snapshot.get("tickets") or []) if row.get("pool_type") == "verifier_pool"],
                        "verifier_receipts": snapshot.get("verifier_receipts") or [],
                        "local_adoption_receipts": [
                            row for row in (snapshot.get("local_adoption_receipts") or [])
                            if str(row.get("pool_type") or "") == "verifier_pool"
                        ],
                        "phase4": snapshot.get("phase4") or {},
                        "phase5": snapshot.get("phase5") or {},
                    })
                return jsonify({"date": "2026-08-05", "error": "coordinator_unavailable"}), 503
            except Exception as e:
                logging.exception("commons/verifier.json error")
                return jsonify({"date": "2026-08-05", "error": str(e)}), 500

        @self.app.route("/commons/adoption.json")
        def commons_adoption_json():
            try:
                limit = int(request.args.get("limit", 100) or 100)
                if self.coordinator and hasattr(self.coordinator, "commons_snapshot"):
                    snapshot = self.coordinator.commons_snapshot(limit=limit)
                    adoptions = snapshot.get("local_adoption_receipts") or []
                    return jsonify({
                        "date": "2026-08-05",
                        "authority": snapshot.get("authority") or {},
                        "local_adoption_receipts": adoptions,
                        "counts": {
                            "total": len(adoptions),
                            "inference_pool": sum(1 for row in adoptions if str(row.get("pool_type") or "") == "inference_pool"),
                            "verifier_pool": sum(1 for row in adoptions if str(row.get("pool_type") or "") == "verifier_pool"),
                        },
                    })
                return jsonify({"date": "2026-08-05", "error": "coordinator_unavailable"}), 503
            except Exception as e:
                logging.exception("commons/adoption.json error")
                return jsonify({"date": "2026-08-05", "error": str(e)}), 500
        ...
        def api_status():
            try:
                ...
                agents = {}
                recent_external_observation = False
                data_store = self.coordinator.agents.get("data_store") if getattr(self.coordinator, "agents", None) else None
                dashboard_reader = None
                if data_store and hasattr(data_store, "open_dashboard_reader"):
                    try:
                        dashboard_reader = data_store.open_dashboard_reader()
                        recent_runs = dashboard_reader.get_observation_runs(limit=1)
                        latest_completed = float((recent_runs[0] if recent_runs else {}).get("completed_ts") or 0.0)
                        recent_external_observation = latest_completed > 0.0 and time.time() - latest_completed <= 300.0
                    except Exception:
                        recent_external_observation = False
                    finally:
                        if dashboard_reader is not None and dashboard_reader is not data_store:
                            try:
                                dashboard_reader.close_dashboard_reader()
                            except Exception:
                                pass
                for name in list(self.coordinator.agents.keys()) if getattr(self.coordinator, "agents", None) else []:
                    ...
                    agent_status = "active" if agent_obj else "unavailable"
                    detail = None
                    if not agent_obj and name in {"market_data", "observation_swarm"} and recent_external_observation:
                        agent_status = "external_active"
                        detail = "Public Kraken observations are arriving through the isolated Phase 1 runner."
                    elif not agent_obj and name == "execution":
                        agent_status = "research_standby"
                        detail = "No live authority; deterministic simulation remains available in Phase 3."
                    elif not agent_obj and name == "wallet":
                        agent_status = "disabled"
                        detail = "Private wallet monitoring is isolated from the UI-only research backend."
                    elif not agent_obj and name == "performance":
                        agent_status = "disabled"
                        detail = "Live P&L accounting is disabled until governed execution is active."
                    elif not agent_obj and name == "dex_margin_oracle" and str(getattr(self.coordinator.cfg, "exchange", "")).lower() != "dex":
                        agent_status = "not_applicable"
                        detail = "The active research venue is Kraken, not a DEX route."
                    agents[name] = {"status": agent_status, "uptime": "-", "last_action": detail}
    ...
    def _render_dashboard(self):
        ...
        symbol = getattr(self.coordinator.cfg, 'symbol', 'PEPE/USDT')
...
<body>
  ...
  <div class="wrap">
	    ...
	    <div class="card integration-deck" id="integrations" style="margin-top:12px;">
	      <div class="integration-head">
	        ...
	        <div class="integration-actions">
	          ...
	          <input id="mainHbSymbol" placeholder="PEPE/USDT" value="$SYMBOL">
    ...
    <div class="card" id="dexCard" style="margin-top:12px; display:$ONCHAIN_DISPLAY;">
      <h3>On-chain Swap Preview</h3>
      <div class="mini" id="dexStatus">No pending route</div>
      ...
      <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:8px;">
        ...
        <button class="btn" onclick="approveSwap()">Sign Swap</button>
      </div>
...
<div class="card" style="margin-top:12px;">
    ...
    <div class="card" id="config" style="margin-top:12px;">
      ...
      <div style="display:flex; gap:8px; flex-wrap:wrap; margin:8px 0;">
        ...
        <button class="btn" onclick="goLive()">Live Locked</button>
      ...
      try{
	    ...
	    async function loadIntegrationDeck(){
	      try{
	        ...
	        if (note) {
	          note.textContent = 'Phoenix authority locked: integrations are research, diagnostic, or sandbox only. Rapid paper owns canary evidence; live activation is absent.';
	        }
	      }catch(e){
	        const note = document.getElementById('mainIntegrationNote');
...
async function loadPerformance() {
    ...
    def _price_series(self, limit: int = 100):
        ...
        if not md:
            ds = self.coordinator.agents.get("data_store") if self.coordinator else None
            if ds and hasattr(ds, "get_observation_price_series"):
                try:
                    symbol = str(getattr(self.coordinator.cfg, "symbol", "PEPE/USD") or "PEPE/USD")
                    rows = ds.get_observation_price_series(symbol=symbol, limit=limit) or []
                    labels = [
        ...
        <style>
            ...
            <form method=\"post\" action=\"/config\">
                <div class=\"card\">
                    ...
                    <label>Symbol</label><input name=\"symbol\" value=\"{val(cfg,'symbol','PEPE/USDT')}\">
    ...
    def _save_config(self):
        try:
            ...
            cfg_updates = {
                ...
                "symbol": data.get("symbol", "PEPE/USDT"),
