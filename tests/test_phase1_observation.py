...
class FakeConfig:
    ...
    phase1_observation_min_interval_sec: int = 30
    phase1_observation_niche_focus_enabled: bool = False
    phase1_observation_niche_quota: int = 0
    phase1_observation_niche_max_volume_usd: float = 2_000_000.0
...
def test_niche_focus_can_prioritize_volatile_small_markets_for_research():
    selector = ObservationOnlyCandidateSelector(
        min_depth_usd_25bps=5_000,
        min_quote_volume_24h_usd=20_000,
        max_spread_bps=100,
        min_data_quality=0.5,
        niche_focus_enabled=True,
        niche_max_quote_volume_24h_usd=2_000_000,
        niche_taker_fee_bps_per_side=10.0,
        niche_min_research_richness=0.2,
        niche_min_volatility_to_cost=0.15,
    )
    major = CandidateObservation(
        symbol='BTC/USD', venue='kraken', timestamp_ms=1, price=50_000.0,
        quote_volume_24h=800_000_000, spread_bps=2.0, depth_usd_25bps=2_000_000,
        listing_age_days=None, venue_count=1, data_quality=1.0, freshness_sec=1.0,
        continuity_ratio=1.0,
        values={
            'volatility_fast': 0.001,
            'volatility_expansion': 1.0,
            'volume_zscore': 0.0,
            'return_zscore': 0.2,
            'trend_slope': 0.000001,
            'range_position': 0.55,
            'regime_inputs': {'regime_hint': 'balanced_transition', 'confidence': 0.4},
        },
    )
    niche = CandidateObservation(
        symbol='WILD/USD', venue='kraken', timestamp_ms=1, price=0.02,
        quote_volume_24h=75_000, spread_bps=80.0, depth_usd_25bps=1_500,
        listing_age_days=None, venue_count=1, data_quality=0.8, freshness_sec=1.0,
        continuity_ratio=1.0,
        values={
            'volatility_fast': 0.03,
            'volatility_expansion': 3.5,
            'volume_zscore': 2.0,
            'return_zscore': 2.4,
            'trend_slope': 0.00012,
            'range_position': 0.96,
    ...
    ranked = selector.rank([major, niche])

    assert ranked[0].symbol == 'WILD/USD'
    assert ranked[0].observation_eligible is False
    assert ranked[0].execution_eligible is False
    assert ranked[0].values['niche_research_candidate'] is True
    assert ranked[0].values['symbol_class'] == 'niche_high_volatility'
    assert ranked[0].values['cohort_bucket'] == 'niche_research'
...
def test_readiness_can_use_recent_hourly_evidence_window(tmp_path: Path):
    store = DataStoreAgent(str(tmp_path / 'recent-readiness.db'))
    coordinator = CaptureCoordinator(store)
    observer = ObservationSwarmAgent(FakeConfig(), FakeClient(), coordinator=coordinator)
    observer.run_once()
    ...
    readiness = store.get_observation_readiness(
        window_hours=1,
        min_distinct_snapshots=1,
        distinct_bucket_hours=1,
        min_mean_quality=0.5,
        min_success_ratio=0.5,
    )

    assert readiness['ready_for_phase2_review'] is True
    assert readiness['distinct_snapshot_buckets'] == 1
    assert readiness['evidence_window_hours'] == 1.0
    assert readiness['execution_eligible'] is False


def test_observer_health_uses_fetch_success_not_ranked_output_cap():
    cfg = FakeConfig()
    cfg.phase1_observation_max_symbols = 1
    cfg.phase1_observation_discovery_pool_multiplier = 3
    observer = ObservationSwarmAgent(cfg, FakeClient(), coordinator=None)

    payload = observer.run_once()

    assert payload['run']['symbols_attempted'] > len(payload['candidates'])
    assert payload['run']['symbols_successful'] == payload['run']['symbols_attempted']
    assert payload['success_ratio'] == 1.0
    assert payload['status'] == 'HEALTHY'


def test_observer_respects_explicit_fast_research_interval_floor():
    cfg = FakeConfig()
    cfg.phase1_observation_interval_sec = 10
    cfg.phase1_observation_min_interval_sec = 1
    observer = ObservationSwarmAgent(cfg, FakeClient(), coordinator=None)

    assert observer.interval_sec == 10


def test_research_only_unlock_allows_niche_profile_without_live_authority(tmp_path: Path):
    settings = tmp_path / 'settings.yaml'
    profile = tmp_path / 'high_vol.yaml'
    settings.write_text(
        '\n'.join([
    ...
    profile.write_text(
        '\n'.join([
            ...
            'research_only_unlock_enabled: true',
            'phase0_quarantine: false',
            'dry_run: true',
            'live_mode: false',
            'auto_trade_enabled: false',
            'auto_trading_enabled: false',
            'onchain_enabled: false',
            'coin_selection_auto_switch: true',
            'public_bot_metrics_auto_promote: true',
            'swarmguard_small_trade_bypass: true',
            'hummingbot_v2_leverage: 1',
            'hummingbot_sidecar_live_enabled: false',
            'phase1_observation_enabled: true',
            'promotion_allowed_stages:',
            '- paper',
            '- research_import',
        ]),
        encoding='utf-8',
    ...
    cfg = load_observer_config(settings, profile)

    assert cfg.profile_name == 'high_vol_low_stakes'
    assert cfg.coin_selection_auto_switch is True
