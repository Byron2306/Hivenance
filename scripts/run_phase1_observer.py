...
def load_observer_config(settings_path: Path, profile_path: Path | None = None) -> SimpleNamespace:
    ...
    profile_name = str(settings.get("profile_name") or "")
    research_only_unlock = bool(
        settings.get("research_only_unlock_enabled")
    ...
    if "auto_trading_enabled" in settings:
        required_safety["auto_trading_enabled"] = False
    if not research_only_unlock:
    ...
    if research_only_unlock:
        allowed_stages = {str(item) for item in (settings.get("promotion_allowed_stages") or [])}
        if not allowed_stages.issubset({"research_import", "paper"}):
            violations.append("research-only unlock may only allow research_import/paper promotion stages")
    if int(settings.get("hummingbot_v2_leverage", 1)) != 1:
        violations.append("hummingbot_v2_leverage must equal 1")
    if bool(settings.get("hummingbot_sidecar_live_enabled", False)):
        violations.append("hummingbot_sidecar_live_enabled must be false")
...
def main() -> int:
    ...
    min_interval = max(1, int(getattr(cfg, "phase1_observation_min_interval_sec", 30) or 30))
    interval = max(min_interval, int(getattr(cfg, "phase1_observation_interval_sec", 120) or 120))
    while not stopping:
