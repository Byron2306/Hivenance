...
def test_consistent_candidate_survives_adversarial_gates():
    ...
    assert champion["candidate_key"].startswith("model_id=breakout_continuation_v1::")
    assert champion["candidate_scope"] == {
...
def test_lucky_concentrated_candidate_is_rejected():
    ...
    breakout = next(
        ...
        if item["candidate_key"] == "model_id=breakout_continuation_v1::order_policy=market"
    )
...
def test_phase4_adopted_verifier_packets_are_scoped_to_validation_run(tmp_path: Path):
    store = DataStoreAgent(str(tmp_path / "phase4_verifier_scope.db"))
    for run_id in ("run-old", "run-current"):
        receipt_id = f"verifier-{run_id}"
        verifier = {
            "receipt_id": receipt_id,
            "ticket_id": f"ticket-{run_id}",
            "phase_scope": 4,
            "pool_type": "verifier_pool",
            "task_class": "phase4_pbo_confirmation",
            "verification_verdict": "PASS",
            "target_object": {"phase4_run_id": run_id, "candidate_key": "candidate::policy"},
        }
        assert store.persist_commons_verifier_receipt(verifier)
        adoption = store.build_commons_local_adoption_receipt(
            source_payload=verifier,
            source_receipt_id=receipt_id,
            source_family="verifier_receipt",
            pool_type="verifier_pool",
            phase_scope=4,
            adopted_by="test",
            adoption_decision="ACCEPTED_VALIDATION_EVIDENCE",
        )
        assert store.persist_commons_local_adoption_receipt(adoption)

    packets = store.get_commons_phase4_adopted_verifier_packets(run_id="run-current")

    assert len(packets) == 1
    assert packets[0]["verifier_receipt"]["target_object"]["phase4_run_id"] == "run-current"
