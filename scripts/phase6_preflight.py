...
def main() -> int:
    ...
    if "HIVENANCE_KRAKEN_API_KEY" not in runner or "HIVENANCE_KRAKEN_API_SECRET" not in runner:
        fail("runner must reference Kraken credential environment variables")
    if "config/api_keys.json" not in runner or "/home/byron/Hivenance/config/api_keys.json" not in runner:
        fail("runner must support both local and home-repo api_keys.json credential fallbacks")
