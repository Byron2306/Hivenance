...
from strategies.volatility_breakout.medium_horizon_trend_lab import run_medium_horizon_trend_lab


def main() -> int:
    parser = argparse.ArgumentParser(description="Hivenance medium-horizon trend research lab")
    ...
    if args.exchange:
        cfg.exchange = str(args.exchange).lower()
    ...
    try:
        summary = run_medium_horizon_trend_lab(
            conn,
            cfg,
            venue=venue,
    ...
    print(
        f"MEDIUM_TREND run={summary.get('run_id')} "
