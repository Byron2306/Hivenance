...
def load_local_exchange_credentials() -> dict[str, str]:
    env_key = os.environ.get("HIVENANCE_KRAKEN_API_KEY", "").strip()
    env_secret = os.environ.get("HIVENANCE_KRAKEN_API_SECRET", "").strip()
    if env_key and env_secret:
        return {"api_key": env_key, "api_secret": env_secret, "source": "environment"}
    candidate_paths = [
        ROOT / "config" / "api_keys.json",
        Path("/home/byron/Hivenance/config/api_keys.json"),
    ]
    for api_keys_path in candidate_paths:
        if not api_keys_path.exists():
            continue
        try:
            payload = json.loads(api_keys_path.read_text(encoding="utf-8")) or {}
        except Exception:
            payload = {}
        file_key = str(payload.get("kraken_api_key") or "").strip()
        file_secret = str(payload.get("kraken_api_secret") or "").strip()
        if file_key and file_secret:
            try:
                source = str(api_keys_path.relative_to(ROOT))
            except ValueError:
                source = str(api_keys_path)
            return {"api_key": file_key, "api_secret": file_secret, "source": source}
    return {"api_key": "", "api_secret": "", "source": "unavailable"}
...
def build_private_client(cfg: Any) -> KrakenRestClient | None:
    ...
    secret = credentials.get("api_secret", "").strip()
