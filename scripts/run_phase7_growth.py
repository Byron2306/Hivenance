...
def main() -> int:
    ...
    if args.approve_next_entry:
        ...
        proof = build_scale_step_proof(canary_store, growth_store, cfg)
        ...
        growth_store.consume_scale_step_proof(
            proof["scale_step_proof_id"],
            phase6_approval_id=approval.approval_id,
        )
        print(json.dumps({
            "approved": True,
            "entry_approval": approval.to_dict(),
            "scale_step_proof": proof,
            "stage": growth_snapshot(canary_store, growth_store, cfg, limit=1)["current_stage"],
            "warning": "One entry only. The growth stage does not remove the per-entry human lease, and each new entry now requires fresh reconciled live evidence.",
        }, indent=2, sort_keys=True, default=str))
        return 0
