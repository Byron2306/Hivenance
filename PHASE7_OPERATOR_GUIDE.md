...
Load trade-only, no-withdrawal Kraken credentials. Preferred order:

1. environment variables for the current operator session
...
Fill only the Kraken fields. The Phase-7 runner reuses the same Phase-6 private-client wiring, so it
will automatically consume either `config/api_keys.json` or
`/home/byron/Hivenance/config/api_keys.json` if the environment variables are absent.
