...
def build_freeze_from_phase4(
    ...
    *,
    ...
    symbol = str(champion.get("symbol") or scope.get("symbol") or "").strip() or None
    direction = str(champion.get("direction") or scope.get("direction") or "").upper().strip() or None
...
class ShadowIntentBuilder:
    ...
    def build(
        ...
        if direction not in {"UP", "DOWN"}:
            raise ValueError("Phase-5 shadow flight only accepts UP or DOWN forecasts")
        frozen_direction = str(freeze.direction or "").upper()
        if frozen_direction and direction != frozen_direction:
            raise ValueError("forecast direction does not match frozen champion")
        frozen_symbol = str(freeze.symbol or "")
        if frozen_symbol and str(forecast.get("symbol") or "") != frozen_symbol:
            raise ValueError("forecast symbol does not match frozen champion")
        ...
        allow_below_min_notional = bool(
            getattr(self.cfg, "phase5_shadow_allow_below_venue_min_notional", False)
            or str(getattr(self.cfg, "profile_name", "") or "") == "high_vol_low_stakes"
        )
        if sizing["quantity"] < profile.min_quantity:
            raise ValueError("shadow intent quantity is below venue minimum")
        if sizing["notional"] < profile.min_notional_usd and not allow_below_min_notional:
        ...
        return ShadowOrderIntent(
            ...
            admission=forecast.get("phase5_shadow_admission") if isinstance(forecast.get("phase5_shadow_admission"), Mapping) else {
                "mode": "phase2_non_abstain_forecast",
                "authority": "research_shadow_only",
                "below_venue_min_notional_research_only": bool(
                    sizing["notional"] < profile.min_notional_usd and allow_below_min_notional
                ),
            },
        )
...
class ShadowSettlementEngine:
    ...
    def settle(
        ...
        *,
        ...
        if passive_fill:
            entry_price = limit_value or entry_mid * (1.0 + entry_spread / 20000.0 if is_down else 1.0 - entry_spread / 20000.0)
            entry_liquidity = "maker"
            entry_slippage_bps = ((reference - entry_price) / reference) * 10000.0 if is_down and reference > 0 else ((entry_price / reference) - 1.0) * 10000.0
        else:
            if is_down:
        ...
        if is_down:
            ...
            exit_slippage_bps = ((exit_price - exit_mid) / exit_mid) * 10000.0 if exit_mid > 0 else 0.0
        else:
            exit_price = exit_mid * (1.0 - (exit_spread / 2.0 + impact_bps) / 10000.0)
        ...
        if is_down:
            gross = ((entry_mid / exit_mid) - 1.0) * 10000.0 if exit_mid > 0 else 0.0
            net = ((entry_price / exit_price) - 1.0) * 10000.0 - fee_bps if exit_price > 0 else 0.0
        else:
