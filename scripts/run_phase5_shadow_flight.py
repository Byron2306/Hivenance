...
def main() -> int:
    ...
    if args.revoke_approval:
        ...
        print(json.dumps({"revoked": ok, "reason": args.reason}, indent=2), flush=True)
    ...
    def collect() -> None:
        ...
        if not args.once:
            print(
                f"PHASE5_CYCLE_START drive_upstream={not args.shadow_only} "
                f"exchange={getattr(cfg, 'exchange', 'kraken')} db={db_path}",
                file=sys.stderr,
                flush=True,
            )
    ...
    interval = max(
        max(1, int(getattr(cfg, "phase5_min_interval_sec", 30) or 30)),
        int(getattr(cfg, "phase5_interval_sec", 120) or 120),
    )
    while not stopping:
        ...
        try:
            ...
            print(f"Phase-5 cycle failed: {type(exc).__name__}: {exc}", file=sys.stderr, flush=True)
