...
class CanaryStore:
    ...
    def _create_tables(self) -> None:
        ...
        try:
            ...
            intent_cols = {row[1] for row in c.execute("PRAGMA table_info(phase6_canary_intents)").fetchall()}
            if "capability_receipt_id" not in intent_cols:
                c.execute("ALTER TABLE phase6_canary_intents ADD COLUMN capability_receipt_id TEXT")
            c.execute('''
            CREATE TABLE IF NOT EXISTS phase6_canary_capability_receipts (
                ...
                expires_ts REAL,
                consumed_ts REAL,
                consumed_by_client_order_id TEXT,
                consumed_by_canary_intent_id TEXT,
                payload TEXT
            )
            ''')
            c.execute(
                "CREATE INDEX IF NOT EXISTS idx_phase6_capability_receipts_status "
                "ON phase6_canary_capability_receipts(status, expires_ts, minted_ts)"
            )
            c.execute('''
            ...
            ''')
            c.execute('''
            CREATE TABLE IF NOT EXISTS phase6_canary_evidence_receipts (
            ...
            ''')
            c.execute(
                "CREATE INDEX IF NOT EXISTS idx_phase6_evidence_receipts_created "
                "ON phase6_canary_evidence_receipts(created_ts, stage_context)"
            )
            c.execute('''
        ...
        with self._lock:
            try:
                self.conn.execute(
                    ...
                    (canary_intent_id,shadow_intent_id,forecast_id,approval_id,freeze_id,capability_receipt_id,client_order_id,
                    ...
                    (
                        ...
                        intent.get("approval_id"), intent.get("freeze_id"), intent.get("capability_receipt_id"),
    ...
    def persist_capability_receipt(self, receipt: Mapping[str, Any]) -> bool:
        with self._lock:
            try:
                self.conn.execute(
                    """INSERT INTO phase6_canary_capability_receipts
                    (capability_receipt_id,approval_id,freeze_id,phase4_run_id,candidate_key,model_id,order_policy,
                     shadow_intent_id,forecast_id,symbol,side,max_notional_usd,config_hash,authority_ceiling,status,
                     minted_ts,expires_ts,consumed_ts,consumed_by_client_order_id,consumed_by_canary_intent_id,payload)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (
                        receipt.get("capability_receipt_id"), receipt.get("approval_id"), receipt.get("freeze_id"),
                        receipt.get("phase4_run_id"), receipt.get("candidate_key"), receipt.get("model_id"),
                        receipt.get("order_policy"), receipt.get("shadow_intent_id"), receipt.get("forecast_id"),
                        receipt.get("symbol"), receipt.get("side"), receipt.get("max_notional_usd"),
                        receipt.get("config_hash"), receipt.get("authority_ceiling"), receipt.get("status"),
                        receipt.get("minted_ts"), receipt.get("expires_ts"), receipt.get("consumed_ts"),
                        receipt.get("consumed_by_client_order_id"), receipt.get("consumed_by_canary_intent_id"),
                        json.dumps(dict(receipt), sort_keys=True, default=str),
                    ),
                )
                self.conn.commit()
                return True
    ...
    def get_capability_receipt(self, capability_receipt_id: str) -> dict[str, Any]:
        return self._fetchone(
            "SELECT * FROM phase6_canary_capability_receipts WHERE capability_receipt_id=?",
            (str(capability_receipt_id or ""),),
        )
    ...
    def consume_capability_receipt(
        ...
        *,
        ...
        with self._lock:
            c = self.conn.cursor()
            c.execute(
                """UPDATE phase6_canary_capability_receipts
                SET status='CONSUMED', consumed_ts=?, consumed_by_client_order_id=?, consumed_by_canary_intent_id=?
                WHERE capability_receipt_id=? AND status='MINTED' AND expires_ts>?""",
                (
                    now_ts,
                    str(client_order_id or ""),
                    str(canary_intent_id or ""),
                    str(capability_receipt_id or ""),
                    now_ts,
                ),
            )
    ...
    def capability_receipts(self, limit: int = 100) -> list[dict[str, Any]]:
        return self._fetchall(
            "SELECT * FROM phase6_canary_capability_receipts ORDER BY minted_ts DESC LIMIT ?",
            (int(limit),),
        )
    ...
    def persist_evidence_receipt(self, receipt: Mapping[str, Any]) -> None:
        with self._lock:
            self.conn.execute(
                """INSERT OR REPLACE INTO phase6_canary_evidence_receipts
                (evidence_receipt_id,created_ts,evidence_scope,authority_ceiling,stage_context,config_hash,evidence_hash,payload)
                VALUES (?,?,?,?,?,?,?,?)""",
                (
                    receipt.get("evidence_receipt_id"),
                    receipt.get("created_ts"),
                    receipt.get("evidence_scope"),
                    receipt.get("authority_ceiling"),
                    receipt.get("stage_context"),
                    receipt.get("config_hash"),
                    receipt.get("evidence_hash"),
                    json.dumps(dict(receipt), sort_keys=True, default=str),
                ),
            )
            self.conn.commit()

    def latest_evidence_receipt(self, *, stage_context: Optional[int] = None) -> dict[str, Any]:
        if stage_context is None:
            return self._fetchone(
                "SELECT * FROM phase6_canary_evidence_receipts ORDER BY created_ts DESC LIMIT 1"
            )
        return self._fetchone(
            "SELECT * FROM phase6_canary_evidence_receipts WHERE stage_context=? ORDER BY created_ts DESC LIMIT 1",
            (int(stage_context),),
        )

    def evidence_receipts(self, limit: int = 50) -> list[dict[str, Any]]:
        return self._fetchall(
            "SELECT * FROM phase6_canary_evidence_receipts ORDER BY created_ts DESC LIMIT ?",
            (int(limit),),
        )

    def paper_canary_settlements(self, limit: int = 50) -> list[dict[str, Any]]:
        return self._fetchall(
            """
            SELECT * FROM phase6_canary_evidence_receipts
            WHERE evidence_scope='phase6_rapid_paper_canary_settlement'
    ...
    def scorecard(self) -> dict[str, Any]:
        ...
        paper_intents = c.execute("SELECT COUNT(*) FROM phase6_canary_intents WHERE status LIKE 'PAPER_CANARY%'").fetchone()[0]
        paper_open = c.execute("SELECT COUNT(*) FROM phase6_canary_intents WHERE status='PAPER_CANARY'").fetchone()[0]
        paper_wins = c.execute("SELECT COUNT(*) FROM phase6_canary_intents WHERE status='PAPER_CANARY_WIN'").fetchone()[0]
        paper_losses = c.execute("SELECT COUNT(*) FROM phase6_canary_intents WHERE status='PAPER_CANARY_LOSS'").fetchone()[0]
        paper_expired = c.execute(
            "SELECT COUNT(*) FROM phase6_canary_intents WHERE status='PAPER_CANARY_EXPIRED_NO_OBSERVATION'"
        ).fetchone()[0]
        paper_pruned = c.execute(
            "SELECT COUNT(*) FROM phase6_canary_intents WHERE status='PAPER_CANARY_PRUNED_BY_OPEN_CAP'"
        ).fetchone()[0]
        paper_receipts = self.paper_canary_settlements(limit=500)
        paper_net = [
            float(row.get("net_return_bps"))
            for row in paper_receipts
            if row.get("net_return_bps") is not None
        ]
        paper_gross = [
            float(row.get("gross_return_bps"))
            for row in paper_receipts
            if row.get("gross_return_bps") is not None
        ]
        return {
            "phase": 6,
            "mode": "phase6_canary",
            ...
            "paper_canary_intents": int(paper_intents or 0),
            "paper_canary_open": int(paper_open or 0),
            "paper_canary_settled": int((paper_wins or 0) + (paper_losses or 0)),
            "paper_canary_wins": int(paper_wins or 0),
            "paper_canary_losses": int(paper_losses or 0),
            "paper_canary_expired": int(paper_expired or 0),
            "paper_canary_pruned": int(paper_pruned or 0),
            "paper_canary_win_rate": round(float(paper_wins or 0) / max(1, int((paper_wins or 0) + (paper_losses or 0))), 6)
            if int((paper_wins or 0) + (paper_losses or 0)) else None,
            "paper_canary_mean_net_bps": round(sum(paper_net) / max(1, len(paper_net)), 6) if paper_net else None,
            "paper_canary_mean_gross_bps": round(sum(paper_gross) / max(1, len(paper_gross)), 6) if paper_gross else None,
