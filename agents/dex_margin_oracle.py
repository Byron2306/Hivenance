...
class DexMarginOracle:
    ...
    def _roundtrip_quote(self, token: Dict[str, Any], amount_usd: float) -> Dict[str, Any]:
        ...
        try:
            ...
            buy = self._get_quote(quote_token["address"], token["address"], amount_in)
            ...
            sell = self._get_quote(token["address"], quote_token["address"], int(token_out))
            ...
            out = {
                ...
                "gas_usd": gas_usd,
                "provider": sell.get("_provider") or buy.get("_provider"),
                "stale": False,
            }
    ...
    def _get_quote(self, src: str, dst: str, amount: int) -> Dict[str, Any]:
        """Try 1inch (if a key is configured), then fall back to keyless
        aggregator APIs (ParaSwap/Velora, then KyberSwap) so quote simulation
        keeps working even without a 1inch API key."""
        errors = []
        if self.api_key:
            try:
                quote = self._oneinch_quote(src, dst, amount)
                quote["_provider"] = "1inch"
                return quote
            except Exception as e:
                errors.append(f"1inch: {e}")
        try:
            quote = self._paraswap_quote(src, dst, amount)
            quote["_provider"] = "paraswap"
            return quote
        except Exception as e:
            errors.append(f"paraswap: {e}")
        try:
            quote = self._kyberswap_quote(src, dst, amount)
            quote["_provider"] = "kyberswap"
            return quote
        except Exception as e:
            errors.append(f"kyberswap: {e}")
        raise RuntimeError("; ".join(errors) or "no quote provider available")

    def _paraswap_quote(self, src: str, dst: str, amount: int) -> Dict[str, Any]:
        """Keyless ParaSwap/Velora price API. No signup required."""
        ...
        dst_amount = route.get("destAmount")
        if dst_amount is None:
            raise RuntimeError("paraswap: missing destAmount")
        return {
    ...
    def _kyberswap_quote(self, src: str, dst: str, amount: int) -> Dict[str, Any]:
        """Keyless KyberSwap Aggregator API. No signup required."""
        chain_slug = self.CHAIN_SLUGS.get(self.chain_id)
        if not chain_slug:
            raise RuntimeError(f"kyberswap: unsupported chain_id {self.chain_id}")
        url = f"https://aggregator-api.kyberswap.com/{chain_slug}/api/v1/routes"
        params = {"tokenIn": src, "tokenOut": dst, "amountIn": str(amount)}
        resp = requests.get(url, params=params, timeout=self.timeout)
        if not (200 <= resp.status_code < 300):
            raise RuntimeError(f"{resp.status_code}: {resp.text[:240]}")
        data = resp.json()
        summary = ((data.get("data") or {}).get("routeSummary")) or {}
        dst_amount = summary.get("amountOut")
        if dst_amount is None:
            raise RuntimeError("kyberswap: missing amountOut")
        return {
            "dstAmount": dst_amount,
            "gas": summary.get("gas"),
            "raw": summary,
