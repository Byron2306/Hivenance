...
class Phase1FeatureEngine:
    ...
    def build(
        ...
        *,
        ...
        worker_points = min(len(closes), len(volumes), self.baseline_window + self.short_window + 15)
        worker_series = {
            'schema': 'hivenance_worker_input_series_v1',
            'source': 'phase1_ohlcv',
            'timeframe': self.timeframe,
            'points': worker_points,
            'closes': [round(float(value), 12) for value in closes[-worker_points:]] if worker_points else [],
            'volumes': [round(float(value), 12) for value in volumes[-worker_points:]] if worker_points else [],
            'latest_price': round(float(price), 12) if price is not None else None,
