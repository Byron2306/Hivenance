...
1. environment variables for short-lived operator sessions
2. ignored local file: `config/api_keys.json`
3. existing Byron home repo file: `/home/byron/Hivenance/config/api_keys.json`

The runner now supports both. Environment variables win if both exist.
...
Do not paste secrets into logs, screenshots, chat, committed YAML, or support archives. The Phase-6
runner now accepts either the local ignored `config/api_keys.json` file or the existing
`/home/byron/Hivenance/config/api_keys.json` file when environment variables are absent.
