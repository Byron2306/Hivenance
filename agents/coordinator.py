...
class SwarmCoordinator:
    def __init__(self, cfg):
        ...
        self._ui_snapshot_cache = {}
        self._init_buzz_governance()

        # Initialize agents
        self._initialize_agents()

    def _get_cached_ui_snapshot(self, key: str, ttl_sec: float) -> Optional[Dict[str, Any]]:
        cached = self._ui_snapshot_cache.get(key)
    ...
    def _set_cached_ui_snapshot(self, key: str, value: Dict[str, Any], ttl_sec: float) -> Dict[str, Any]:
        payload = value if isinstance(value, dict) else {}
        self._ui_snapshot_cache[key] = {
            "expires_at": time.time() + max(0.1, float(ttl_sec or 0.1)),
            "value": payload,
        }
        return payload
    ...
    def observation_snapshot(self, symbol: Optional[str] = None, limit: int = 100, compact: bool = False) -> Dict[str, Any]:
        ...
        primary_ds = self.agents.get("data_store") if hasattr(self, "agents") else None
        ds = primary_ds.open_dashboard_reader() if compact and primary_ds and hasattr(primary_ds, "open_dashboard_reader") else primary_ds
        history = []
        runs = []
        ...
        if ds:
            ...
            try:
                readiness = ds.get_observation_readiness(
                    required_days=float(getattr(self.cfg, "phase1_readiness_required_days", 7.0) or 7.0),
                    ...
                    window_hours=float(getattr(self.cfg, "phase1_readiness_window_hours", 1.0) or 1.0),
                    min_distinct_snapshots=int(getattr(self.cfg, "phase1_readiness_min_distinct_snapshots", 1) or 1),
                    distinct_bucket_hours=int(getattr(self.cfg, "phase1_readiness_distinct_snapshot_bucket_hours", 1) or 1),
        ...
        if compact and latest:
            ...
            persisted_status = str(latest.get("status") or "PERSISTED")
            if status_payload.get("status") in {"UNAVAILABLE", "DEGRADED", "OFFLINE", "WAITING"}:
                status_payload = {
                    "phase": 1,
        ...
        elif status_payload.get("status") == "UNAVAILABLE" and latest:
            latest_run = latest.get("run") if isinstance(latest.get("run"), dict) else {}
            status_payload = {
                "phase": 1,
                "mode": "observation_only",
        ...
        if ds is not primary_ds and hasattr(ds, "close_dashboard_reader"):
            ds.close_dashboard_reader()
        return result
    ...
    def hypothesis_snapshot(
        ...
        cache_key = f"hypothesis_snapshot:{model_id or '*'}:{symbol or '*'}:{int(limit or 250)}:{1 if compact else 0}"
        if compact:
            cached = self._get_cached_ui_snapshot(cache_key, ttl_sec=10.0)
            if cached:
                return cached
        def _compact_hypothesis_forecast_rows(rows: Any, max_rows: int = 24) -> list[Dict[str, Any]]:
            ...
            for row in list(rows or [])[:max_rows]:
                ...
                compact_row = {
                    ...
                    "target_timestamp_ms": row.get("target_timestamp_ms"),
                    ...
                    "crystal_memory": row.get("crystal_memory") if isinstance(row.get("crystal_memory"), dict) else {},
                ...
                compact_inputs = {
                    "regime_hint": inputs.get("regime_hint"),
                    "cohort_bucket": inputs.get("cohort_bucket"),
                    "spread_bps": inputs.get("spread_bps"),
                    "depth_usd_25bps": inputs.get("depth_usd_25bps"),
                    "volatility_expansion": inputs.get("volatility_expansion"),
                    "return_zscore": inputs.get("return_zscore"),
                    "volume_zscore": inputs.get("volume_zscore"),
                    "adaptive_policy": inputs.get("adaptive_policy") if isinstance(inputs.get("adaptive_policy"), dict) else None,
                    "cost": inputs.get("cost") if isinstance(inputs.get("cost"), dict) else None,
                    "federation": inputs.get("federation") if isinstance(inputs.get("federation"), dict) else None,
                }
                compact_row["inputs"] = {key: value for key, value in compact_inputs.items() if value is not None}
                compact_rows.append(compact_row)
            return compact_rows

        def _build_symbol_focus_rows(rows: Any) -> list[Dict[str, Any]]:
            focus_rows: list[Dict[str, Any]] = []
            forecast_rows = [row for row in (rows or []) if isinstance(row, dict)]
        ...
        def _build_champion_rows(scorecard_payload: Dict[str, Any]) -> tuple[list[Dict[str, Any]], Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
            leaderboard_rows = sorted(
                [row for row in scorecard_payload.get("models", []) if isinstance(row, dict)],
                key=lambda item: (
            ...
            champion_row = next(
                (row for row in leaderboard_rows if row.get("model_id") == scorecard_payload.get("champion_by_mean_net_bps")),
                leaderboard_rows[0] if leaderboard_rows else None,
            )
            champion_research_row = next(
                (row for row in leaderboard_rows if row.get("model_id") == scorecard_payload.get("champion_research_model_by_mean_net_bps")),
                None,
            )
            return leaderboard_rows, champion_row, champion_research_row
        ...
        if ds:
            if compact:
                try:
                    runs = ds.get_hypothesis_runs(limit=1) if hasattr(ds, "get_hypothesis_runs") else []
                except Exception:
                    runs = []
            else:
                try:
            ...
            if not compact:
                try:
                    ...
                    outcomes = []
            # Fetched in both modes now (not just non-compact): the outer
            # cache_key lookup at the top of this method already bounds this
            # to at most one recompute per ttl_sec=10.0 window in compact mode,
            # so this is no more expensive than the non-compact path already is.
            try:
            ...
            if not compact:
                try:
                    readiness = ds.get_phase2_readiness(
                        ...
                        distinct_bucket_hours=int(getattr(self.cfg, "phase2_readiness_distinct_snapshot_bucket_hours", 24) or 24),
                    ) if hasattr(ds, "get_phase2_readiness") else readiness
                except Exception:
                    pass
        crystal_registry = (
            ds.get_crystal_registry_overview(phase_scope=2)
            if ds and hasattr(ds, "get_crystal_registry_overview")
            else (
                ds.get_crystal_registry_snapshot(limit=12)
                if ds and hasattr(ds, "get_crystal_registry_snapshot")
                else {}
            )
        )
        rung_ledger = (
            ds.get_inference_rung_overview(phase_scope=2)
            if ds and hasattr(ds, "get_inference_rung_overview")
            else (
                ds.get_inference_rung_summary(limit=24, phase_scope=2)
                if ds and hasattr(ds, "get_inference_rung_summary")
                else {}
            )
        )
        if not latest and runs:
            latest = dict((runs[0].get("payload") or {}))
        if compact and isinstance(latest, dict):
            compact_latest = dict(latest)
            compact_forecasts = _compact_hypothesis_forecast_rows(compact_latest.get("forecasts") or [])
            # Built from the full (untruncated) latest-run forecast list, not the
            # 24-row-capped compact_forecasts above, so symbol clustering reflects
            # the whole run rather than just the ledger preview slice.
            compact_symbol_focus = _build_symbol_focus_rows(compact_latest.get("forecasts") or [])
            _, compact_champion_model, compact_champion_research_model = _build_champion_rows(scorecard)
            compact_cohort_breakdown = scorecard.get("cohort_breakdown") or {}
            compact_abstention_reason_breakdown = [row for row in scorecard.get("abstention_reason_breakdown", []) if isinstance(row, dict)][:8]
            compact_near_miss_recovery_candidates = [row for row in scorecard.get("near_miss_recovery_candidates", []) if isinstance(row, dict)][:8]
            compact_demotion_recommendations = [row for row in scorecard.get("demotion_recommendations", []) if isinstance(row, dict)][:8]
            compact_run = dict(compact_latest.get("run") or {})
            compact_federation = dict(compact_latest.get("federation") or {})
            compact_competition = dict(compact_latest.get("competition") or {})
            try:
                compact_commons = ds.get_commons_phase_summary(2, limit=20) if ds and hasattr(ds, "get_commons_phase_summary") else {}
            except Exception:
                compact_commons = {}
            persisted_readiness = compact_latest.get("readiness")
            compact_readiness = dict(persisted_readiness) if isinstance(persisted_readiness, dict) else {
                "phase": 2,
                "ready_for_phase3_review": False,
                "execution_eligible": False,
                "reasons": ["acceptance_snapshot_waiting_for_next_phase2_cycle"],
            }
            persisted_gate = compact_latest.get("dio_gate")
            compact_dio_gate = dict(persisted_gate) if isinstance(persisted_gate, dict) else {
                ...
                "reasons": ["acceptance_snapshot_waiting_for_next_phase2_cycle"],
                "predicates": {},
                "commons_pool": compact_commons,
            }
            compact_summary = {
                "phase": 2,
                "champion_model": compact_champion_model,
                "champion_research_model": compact_champion_research_model,
                "leading_symbol": compact_symbol_focus[0] if compact_symbol_focus else None,
                "symbol_focus": compact_symbol_focus[:12],
                "cohort_breakdown": compact_cohort_breakdown,
                "abstention_reason_breakdown": compact_abstention_reason_breakdown,
                "near_miss_recovery_candidates": compact_near_miss_recovery_candidates,
                "demotion_recommendations": compact_demotion_recommendations,
                "why_now": [
                    f"{int(compact_run.get('forecasts_total') or 0)} forecasts were generated in the latest persisted Phase 2 run.",
                    f"{int(compact_run.get('non_abstain_forecasts') or 0)} were non-abstain across {int(compact_run.get('symbols_evaluated') or 0)} symbols.",
                    f"{int((rung_ledger.get('total') or 0))} Phase 2 inference-rung decisions are currently ledgered.",
                    f"{int((crystal_registry.get('total') or 0))} Phase 2 crystals are indexed for research reuse.",
                    f"Commons has adopted {int(compact_commons.get('adoption_receipts_accepted') or 0)} bounded Phase 2 response(s).",
                    "Compact dashboard snapshot is active to keep the page responsive.",
                ],
                "inference_rung_ledger": {
                    "total": int(rung_ledger.get("total") or 0),
                    "by_rung": rung_ledger.get("by_rung") or {},
                    "by_outcome": rung_ledger.get("by_outcome") or {},
                },
                "crystal_registry": {
                    "total": int(crystal_registry.get("total") or 0),
                    "counts": crystal_registry.get("counts") or {},
                },
            }
            compact_status = {
                "phase": 2,
                ...
                "status": str(compact_latest.get("status") or "PERSISTED"),
                "run_count": len(runs),
                "last_error": None,
                "thread_alive": False,
                "horizons_seconds": list(compact_latest.get("horizons_seconds") or []),
                "primary_models": list(compact_run.get("primary_models") or []),
                "federated_models": list(compact_run.get("federated_models") or []),
                "baseline_models": list(compact_run.get("baseline_models") or []),
                "federation": compact_federation,
                "execution_wired": False,
                "orders_submitted": 0,
                "source": "persisted_hypothesis_runs",
            }
            compact_latest = {
                "status": compact_latest.get("status"),
                "dataset_hash": compact_latest.get("dataset_hash"),
                "timestamp_ms": compact_latest.get("timestamp_ms"),
                "run": compact_run,
                "federation": compact_federation,
                "competition": compact_competition,
                "horizons_seconds": list(compact_latest.get("horizons_seconds") or []),
            }
            result = {
                "phase": 2,
                ...
                "dio_gate": compact_dio_gate,
                "commons_pool": {},
                "crystal_registry": crystal_registry,
                "inference_rung_ledger": rung_ledger,
            }
            if ds is not primary_ds and hasattr(ds, "close_dashboard_reader"):
                ds.close_dashboard_reader()
            return self._set_cached_ui_snapshot(cache_key, result, ttl_sec=10.0)
        status_payload = (
            {"status": str(latest.get("status") or "PERSISTED"), "source": "persisted_hypothesis_runs", "thread_alive": False}
            if compact and latest else
        ...
        leaderboard, champion_model, champion_research_model = _build_champion_rows(scorecard)
        forecast_rows = [row for row in forecasts if isinstance(row, dict)]
        symbol_focus = _build_symbol_focus_rows(forecast_rows)
        ...
        dio_gate = ds.get_dio_gate_snapshot(2) if ds and hasattr(ds, "get_dio_gate_snapshot") else {}
        result = {
            "phase": 2,
            "mode": "hypothesis_research_only",
            "execution_wired": False,
            ...
            "summary": summary,
            "dio_gate": dio_gate,
            "commons_pool": dio_gate.get("commons_pool") if isinstance(dio_gate, dict) else {},
            "crystal_registry": crystal_registry,
            "inference_rung_ledger": rung_ledger,
        }
        if ds is not primary_ds and hasattr(ds, "close_dashboard_reader"):
            ds.close_dashboard_reader()
        return self._set_cached_ui_snapshot(cache_key, result, ttl_sec=2.0) if compact else result
    ...
    def execution_lab_snapshot(
        ...
        cache_key = f"execution_lab_snapshot:{model_id or '*'}:{symbol or '*'}:{int(limit or 250)}:{1 if compact else 0}"
        if compact:
            cached = self._get_cached_ui_snapshot(cache_key, ttl_sec=2.0)
            if cached:
                return cached

        def _build_execution_robustness_ranking(rows: list[Dict[str, Any]]) -> list[Dict[str, Any]]:
        ...
        def _build_best_normal_policy(rows: list[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        def _build_candidate_signals(candidate_intake_payload: Dict[str, Any]) -> tuple[Dict[str, Any], Dict[str, Any]]:
            candidate_sample = candidate_intake_payload.get("sample") if isinstance(candidate_intake_payload, dict) else []
            candidate_sample = candidate_sample if isinstance(candidate_sample, list) else []
            ...
            rejection_counts = candidate_intake_payload.get("rejections") if isinstance(candidate_intake_payload, dict) else {}
            failure_veto_candidates = [
                row for row in candidate_sample
                if isinstance(row, dict) and bool(row.get("failure_veto_active"))
            ]
            failure_veto_candidates.sort(
                key=lambda row: (
                    float(row.get("failure_feedback_dominant_cause_rate") or 0.0),
                    float(row.get("failure_feedback_penalty_bps_equiv") or 0.0),
                    -float(row.get("shrunk_recent_slice_historical_mean_net_bps") or 10**9),
                ),
                reverse=True,
            )
            symbol_class_specialists = [
                row for row in candidate_sample
                if isinstance(row, dict) and row.get("shrunk_symbol_class_historical_mean_net_bps") is not None
            ]
            symbol_class_specialists.sort(
                key=lambda row: (
            ...
            candidate_pressure = {
                "raw_pool_size": int(candidate_intake_payload.get("raw_pool_size", 0) or 0) if isinstance(candidate_intake_payload, dict) else 0,
                "selected_count": int(candidate_intake_payload.get("selected_count", 0) or 0) if isinstance(candidate_intake_payload, dict) else 0,
                "rejections": rejection_counts if isinstance(rejection_counts, dict) else {},
                "failure_veto_candidates": len(failure_veto_candidates),
                "top_failure_veto_candidate": failure_veto_candidates[0] if failure_veto_candidates else None,
                "failure_veto_sample": failure_veto_candidates[:8],
                "top_symbol_class_specialist": symbol_class_specialists[0] if symbol_class_specialists else None,
                "symbol_class_specialists": symbol_class_specialists[:8],
            }
            return candidate_pressure, recovery_signals
        ...
        if ds:
            try:
                ...
                runs = []
            if compact and runs:
                latest_row = dict(runs[0]) if isinstance(runs[0], dict) else {}
                latest_payload = latest_row.get("payload")
                if isinstance(latest_payload, str):
                    try:
                        latest_payload = json.loads(latest_payload or "{}")
                    except Exception:
                        latest_payload = {}
                latest = dict(latest_payload) if isinstance(latest_payload, dict) else latest_row
            if not compact:
                latest = lab.latest_snapshot() if lab and hasattr(lab, "latest_snapshot") else latest
                try:
                ...
                # Bounded/index-friendly recent-window scorecard (LIMIT-based, not
                # an unbounded full-table payload scan like get_execution_scorecard),
                # already proven safe by CURRENT_TRUTH.json rendering. Recompute is
                # itself capped to at most once per ttl_sec=2.0 by the cache_key
                # lookup at the top of this method.
                try:
                    from strategies.volatility_breakout.current_truth import _compact_phase3_scorecard
                    scorecard = _compact_phase3_scorecard(ds, limit=20000)
                except Exception:
                    pass
            try:
                if compact:
                    ...
                    if not isinstance(latest_run, dict):
                        latest_run = {}
                    completed = int(latest_run.get("completed") or 0)
                    readiness = {
                        "phase": 3,
                        "ready_for_phase4_review": False,
                        "execution_eligible": False,
                        "completed_normal_simulations": completed,
                        "reasons": ["compact_snapshot_readiness_not_recomputed"],
                    }
                else:
                ...
                pass
        crystal_registry = (
            ds.get_crystal_registry_overview(phase_scope=3)
            if ds and hasattr(ds, "get_crystal_registry_overview")
            else (
                ds.get_crystal_registry_snapshot(limit=12)
                if ds and hasattr(ds, "get_crystal_registry_snapshot")
        ...
        rung_ledger = (
            ds.get_inference_rung_overview(phase_scope=3)
            if ds and hasattr(ds, "get_inference_rung_overview")
            else (
                ds.get_inference_rung_summary(limit=24, phase_scope=3)
                if ds and hasattr(ds, "get_inference_rung_summary")
                else {}
            )
        )
        ...
        if compact and isinstance(latest, dict):
            compact_latest = dict(latest)
            compact_run = dict(compact_latest.get("run") or {})
            compact_candidate_intake = dict(compact_latest.get("candidate_intake") or {})
            compact_commons = dict(compact_latest.get("commons_inference_pool") or {})
            compact_rejections = dict(compact_candidate_intake.get("rejections") or {})
            compact_selected = int(compact_candidate_intake.get("selected_count") or 0)
            compact_raw_pool = int(compact_candidate_intake.get("raw_pool_size") or 0)
            compact_top_rejections = sorted(
                (
                    {"cause": str(cause), "count": int(count or 0)}
                    for cause, count in compact_rejections.items()
                ),
            ...
            compact_readiness = dict(persisted_readiness) if isinstance(persisted_readiness, dict) else {
                ...
                "ready_for_phase4_review": False,
                "execution_eligible": False,
                "completed_normal_simulations": int(compact_run.get("completed") or 0),
                "reasons": ["acceptance_snapshot_waiting_for_next_phase3_cycle"],
            }
            persisted_gate = compact_latest.get("dio_gate")
            compact_dio_gate = dict(persisted_gate) if isinstance(persisted_gate, dict) else {
                "schema": "dio_gate_snapshot_v1",
            ...
            compact_rows = [row for row in scorecard.get("rows", []) if isinstance(row, dict)]
            compact_robustness_ranking = _build_execution_robustness_ranking(compact_rows)
            compact_best_normal = _build_best_normal_policy(compact_rows)
            compact_candidate_pressure, compact_recovery_signals = _build_candidate_signals(compact_candidate_intake)
            compact_scenario_count = len({str(row.get("scenario") or "") for row in compact_rows if row.get("scenario")})
            compact_policy_count = len({str(row.get("order_policy") or "") for row in compact_rows if row.get("order_policy")})
            compact_completed_outcomes = sum(int(row.get("completed") or 0) for row in compact_rows)
            compact_profitable_outcomes = sum(
                int(row.get("completed") or 0) * float(row.get("win_rate") or 0.0)
                for row in compact_rows
                if row.get("win_rate") is not None
            )
            compact_slice_breakdown = [row for row in scorecard.get("slice_execution_breakdown", []) if isinstance(row, dict)]
            compact_summary = {
                "phase": 3,
                "best_normal_policy": compact_best_normal,
                "best_research_policy": next((row for row in compact_robustness_ranking if not row.get("is_baseline")), None),
                "scenario_count": compact_scenario_count or len(compact_latest.get("scenarios") or []),
                "policy_count": compact_policy_count or len(compact_latest.get("policies") or []),
                "completed_outcomes": compact_completed_outcomes or int(compact_run.get("completed") or 0),
                "profitable_outcomes": int(round(compact_profitable_outcomes)),
                "profitable_rate": round(compact_profitable_outcomes / compact_completed_outcomes, 6) if compact_completed_outcomes else 0.0,
                "adaptation_breakdown": {},
                "slice_execution_breakdown": compact_slice_breakdown[:12],
                "failure_cause_breakdown": [],
                "symbol_class_breakdown": [],
                "robustness_ranking": compact_robustness_ranking[:12],
                "research_winners": [row for row in compact_robustness_ranking if not row.get("is_baseline")][:8],
                "candidate_intake": compact_candidate_intake,
                "candidate_pressure": {
                    **compact_candidate_pressure,
                    "selection_rate": (compact_selected / compact_raw_pool) if compact_raw_pool else 0.0,
                    "top_rejections": compact_top_rejections,
                },
                "recovery_signals": compact_recovery_signals,
                "capabilities": [
                ...
                "why_now": [
                    f"{int(compact_run.get('forecasts_examined') or 0)} forecasts were examined in the latest persisted Phase 3 run.",
                    f"{int(compact_run.get('simulations_created') or 0)} simulations were created and {int(compact_run.get('completed') or 0)} completed.",
                    f"{compact_raw_pool} candidates entered the intake pool and {compact_selected} survived the Phase 3 gate.",
                    f"Commons inference tickets issued: {int(compact_commons.get('tickets_issued') or 0)}.",
                    f"{int((rung_ledger.get('total') or 0))} Phase 3 inference-rung decisions are currently ledgered.",
                    f"{int((crystal_registry.get('total') or 0))} Phase-scoped crystals are indexed for rapid reuse.",
                    "Compact dashboard snapshot is active to keep the page responsive.",
                ],
                "inference_rung_ledger": {
                    "total": int(rung_ledger.get("total") or 0),
            ...
            result = {
                ...
                "latest": compact_latest,
                "runs": runs[:1],
                "orders": list(compact_latest.get("simulations") or []),
                "scorecard": scorecard,
                "readiness": compact_readiness,
                "summary": compact_summary,
                "dio_gate": compact_dio_gate,
            ...
            return self._set_cached_ui_snapshot(cache_key, result, ttl_sec=10.0)
        status_payload = (
            {"status": str(latest.get("status") or "PERSISTED"), "source": "persisted_execution_runs", "thread_alive": False}
            if compact and latest else
        ...
        failure_cause_breakdown = [row for row in scorecard.get("failure_cause_breakdown", []) if isinstance(row, dict)]
        symbol_class_breakdown = [row for row in scorecard.get("symbol_class_breakdown", []) if isinstance(row, dict)]
        ...
        rejection_counts = candidate_intake.get("rejections") if isinstance(candidate_intake, dict) else {}
        failure_veto_candidates = [
            row for row in candidate_sample
            if isinstance(row, dict) and bool(row.get("failure_veto_active"))
        ]
        ...
        candidate_pressure = {
            "raw_pool_size": int(candidate_intake.get("raw_pool_size", 0) or 0) if isinstance(candidate_intake, dict) else 0,
            "selected_count": int(candidate_intake.get("selected_count", 0) or 0) if isinstance(candidate_intake, dict) else 0,
            "rejections": rejection_counts if isinstance(rejection_counts, dict) else {},
            "failure_veto_candidates": len(failure_veto_candidates),
            "top_failure_veto_candidate": failure_veto_candidates[0] if failure_veto_candidates else None,
            "failure_veto_sample": failure_veto_candidates[:8],
        ...
        summary = {
            ...
            "failure_cause_breakdown": failure_cause_breakdown[:12],
            "symbol_class_breakdown": symbol_class_breakdown[:12],
            ...
            "why_now": [
                ...
                (
                    f"{int((rejection_counts or {}).get('failure_cause_veto', 0) or 0)} candidates were vetoed by repeated failure-cause learning."
                    if candidate_intake else
                    "Failure-cause veto pressure is waiting for candidate intake."
                ),
                (
                    f"Best symbol-class specialist is {symbol_class_specialists[0].get('symbol_class') or 'unknown'} at "
                    f"{float(symbol_class_specialists[0].get('shrunk_symbol_class_historical_mean_net_bps') or 0.0):.2f} shrunk net bps."
                    if symbol_class_specialists else
                    "Symbol-class specialization has not identified a clear leader yet."
                ),
                (
        ...
        dio_gate = ds.get_dio_gate_snapshot(3) if ds and hasattr(ds, "get_dio_gate_snapshot") else {}
        result = {
            "phase": 3,
            "mode": "execution_simulation_only",
            "execution_wired": False,
    ...
    def validation_lab_snapshot(self, limit: int = 100, compact: bool = False) -> Dict[str, Any]:
        cache_key = f"validation_lab_snapshot:{int(limit or 100)}:{1 if compact else 0}"
        if compact:
            cached = self._get_cached_ui_snapshot(cache_key, ttl_sec=2.0)
            if cached:
                return cached
        ...
        status_payload = (
            {"status": str(latest.get("status") or "PERSISTED"), "source": "persisted_validation_runs", "thread_alive": False}
            if compact and latest else
            (lab.status() if lab and hasattr(lab, "status") else {"status": "UNAVAILABLE"})
        )
        ...
        dio_gate = ds.get_dio_gate_snapshot(4) if ds and hasattr(ds, "get_dio_gate_snapshot") else {}
        result = {
            "phase": 4,
            "mode": "adversarial_validation_only",
            "execution_wired": False,
    ...
    def shadow_flight_snapshot(self, limit: int = 250, compact: bool = False) -> Dict[str, Any]:
        cache_key = f"shadow_flight_snapshot:{int(limit or 250)}:{1 if compact else 0}"
        if compact:
            cached = self._get_cached_ui_snapshot(cache_key, ttl_sec=2.0)
            if cached:
                return cached
        ...
        primary_ds = self.agents.get("data_store") if hasattr(self, "agents") else None
        ds = primary_ds.open_dashboard_reader() if primary_ds and hasattr(primary_ds, "open_dashboard_reader") else primary_ds
        ...
        dio_gate = ds.get_dio_gate_snapshot(5) if ds and hasattr(ds, "get_dio_gate_snapshot") else {}
        result = {
            "phase": 5,
    ...
    def canary_snapshot(self, limit: int = 100) -> Dict[str, Any]:
        ...
        base = {
            "phase": 6,
            "mode": "rapid_paper_canary" if bool(getattr(self.cfg, "phase6_rapid_paper_canary_enabled", False)) else "tiny_live_canary",
            "operator_process_only": True,
            "desktop_arming_enabled": False,
            "automatic_scaling": False,
            "execution_scale_authorized": False,
            "human_review_required": not bool(getattr(self.cfg, "phase6_rapid_paper_canary_enabled", False)),
            "live_config_enabled": bool(getattr(self.cfg, "phase6_live_submission_enabled", False)),
            "rapid_paper_canary_enabled": bool(getattr(self.cfg, "phase6_rapid_paper_canary_enabled", False)),
            ...
            "allowed_directions": list(getattr(self.cfg, "phase6_allowed_directions", ["UP"]) or ["UP"]),
            ...
            "incidents": [], "reconciliations": [], "paper_canary_settlements": [],
        ...
        try:
            ...
            base.update({
                ...
                "paper_canary_settlements": store.paper_canary_settlements(limit=min(50, int(limit or 100))),
    ...
    def growth_snapshot(self, limit: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            result = growth_snapshot(CanaryStore(ds), GrowthStore(ds), self.cfg, limit=int(limit or 50))
        except Exception as exc:
    ...
    def commons_snapshot(self, limit: int = 100) -> Dict[str, Any]:
        ds = self.agents.get("data_store") if hasattr(self, "agents") else None
        base = {
            "date": "2026-08-05",
            "commons_pool_enabled": False,
            "authority": {
                "execution_authority": "none",
                "promotion_authority": "none",
                "resume_authority": "none",
                "scaling_authority": "none",
                "bridge_authority": "local_adoption_receipt_only",
            },
            "tickets": [],
            "leases": [],
            "inference_receipts": [],
            "verifier_receipts": [],
            "local_adoption_receipts": [],
            "phase2": {"adopted_packets": 0},
            "phase3": {"adopted_packets": 0},
            "phase4": {"adopted_packets": 0},
            "phase5": {"adopted_packets": 0},
        }
        if not ds or not hasattr(ds, "get_commons_pool_snapshot"):
            return base
        try:
            bounded_limit = min(250, int(limit or 100))
            if hasattr(ds, "get_commons_dashboard_snapshot"):
                snapshot = ds.get_commons_dashboard_snapshot(limit=bounded_limit)
            else:
                snapshot = ds.get_commons_pool_snapshot(limit=bounded_limit)
            storage_authority = snapshot.pop("authority", None)
            phase2_packets = snapshot.pop("phase2_packets", None)
            phase3_packets = snapshot.pop("phase3_packets", None)
            phase4_packets = snapshot.pop("phase4_packets", None)
            phase5_packets = snapshot.pop("phase5_packets", None)
            now_ts = time.time()
            tickets = list(snapshot.get("tickets") or [])
            completed_ticket_ids = {
                str(row.get("ticket_id") or "")
                for row in [
                    *(snapshot.get("inference_receipts") or []),
                    *(snapshot.get("verifier_receipts") or []),
                ]
                if row.get("ticket_id")
            }
            active_tickets = [
                row for row in tickets
                if str(row.get("status") or "ISSUED").upper() in {"ISSUED", "OPEN", "CLAIMABLE"}
                and float(row.get("expires_ts") or 0.0) > now_ts
                and str(row.get("ticket_id") or "") not in completed_ticket_ids
            ]
            expired_tickets = [
                row for row in tickets
                if float(row.get("expires_ts") or 0.0) <= now_ts
            ]
            phase2_packets = phase2_packets if phase2_packets is not None else (ds.get_commons_phase2_adopted_challenger_packets(limit=bounded_limit) if hasattr(ds, "get_commons_phase2_adopted_challenger_packets") else [])
            phase3_packets = phase3_packets if phase3_packets is not None else (ds.get_commons_phase3_adopted_execution_packets(limit=bounded_limit) if hasattr(ds, "get_commons_phase3_adopted_execution_packets") else [])
            phase4_packets = phase4_packets if phase4_packets is not None else (ds.get_commons_phase4_adopted_verifier_packets(limit=bounded_limit) if hasattr(ds, "get_commons_phase4_adopted_verifier_packets") else [])
            phase5_packets = phase5_packets if phase5_packets is not None else (ds.get_commons_phase5_adopted_verifier_packets(limit=bounded_limit) if hasattr(ds, "get_commons_phase5_adopted_verifier_packets") else [])
            base.update(snapshot)
            base["commons_pool_enabled"] = True
            base["storage_authority"] = storage_authority or "research_and_verification_only"
            base["ticket_state"] = {
                "active": len(active_tickets),
                "expired": len(expired_tickets),
                "completed": sum(1 for row in tickets if str(row.get("ticket_id") or "") in completed_ticket_ids),
                "total_returned": len(tickets),
                "active_leases": sum(
                    1 for row in (snapshot.get("leases") or [])
                    if float(row.get("expires_ts") or 0.0) > now_ts
                    and str(row.get("ticket_id") or "") not in completed_ticket_ids
                ),
            }
            base["phase2"] = {
                "adopted_packets": len(phase2_packets),
                "recent": phase2_packets[:10],
            }
            base["phase3"] = {
                "adopted_packets": len(phase3_packets),
                "recent": phase3_packets[:10],
            }
            base["phase4"] = {
                "adopted_packets": len(phase4_packets),
                "recent": phase4_packets[:10],
                "contradictions": sum(
                    1
                    for item in phase4_packets
                    if str(((item.get("verifier_receipt") or {}).get("verification_verdict") or "")).upper() in {"FAIL", "CONTRADICT"}
                ),
            }
            base["phase5"] = {
                "adopted_packets": len(phase5_packets),
                "recent": phase5_packets[:10],
                "contradictions": sum(
                    1
                    for item in phase5_packets
                    if str(((item.get("verifier_receipt") or {}).get("verification_verdict") or "")).upper() in {"FAIL", "CONTRADICT"}
                ),
            }
            return base
        except Exception as exc:
            logging.exception("Commons snapshot failed")
