...
def _build_growth_evidence_payload(canary: CanaryStore, growth: GrowthStore, cfg: Any) -> dict[str, Any]:
...
def seal_phase6_evidence(canary: CanaryStore, growth: GrowthStore, cfg: Any) -> dict[str, Any]:
    evidence = _build_growth_evidence_payload(canary, growth, cfg)
    created_ts = time.time()
    receipt = {
        "evidence_receipt_id": canonical_hash(
            {
                "stage_context": evidence.get("current_stage"),
                "stage_started_ts": evidence.get("stage_started_ts"),
                "completed_round_trips": evidence.get("completed_round_trips"),
                "new_round_trips": evidence.get("new_round_trips"),
                "evidence_hash": evidence.get("evidence_hash"),
            }
        ),
        "created_ts": created_ts,
        "evidence_scope": "phase6_live_outcomes_for_phase7_review",
        "authority_ceiling": "evidence_only",
        "stage_context": int(evidence.get("current_stage", 0) or 0),
        "config_hash": phase7_config_hash(cfg),
        "evidence_hash": evidence.get("evidence_hash"),
        "evidence": evidence,
        "execution_scale_authorized": False,
        "promotion_authority": "none",
        "live_order_authority": "phase6_operator_only",
    }
    canary.persist_evidence_receipt(receipt)
    return receipt


def growth_evidence(canary: CanaryStore, growth: GrowthStore, cfg: Any) -> dict[str, Any]:
    return dict(seal_phase6_evidence(canary, growth, cfg).get("evidence") or {})


def build_scale_step_proof(canary: CanaryStore, growth: GrowthStore, cfg: Any, *, now_ts: Optional[float] = None) -> dict[str, Any]:
    now = float(now_ts or time.time())
    state = growth.get_state()
    current_stage = int(state.get("current_stage", 0) or 0)
    if str(state.get("state") or "").upper() != "ACTIVE" or current_stage < 1:
        raise ValueError("an active Phase-7 stage is required before minting a scale-step proof")
    receipt = seal_phase6_evidence(canary, growth, cfg)
    if receipt.get("authority_ceiling") != "evidence_only":
        raise ValueError("Phase-6 evidence receipt authority ceiling is invalid")
    evidence = dict(receipt.get("evidence") or {})
    latest = growth.latest_scale_step_proof(stage_id=current_stage)
    latest_rt = int(latest.get("completed_round_trips", 0) or 0) if latest else int(state.get("stage_start_round_trips", 0) or 0)
    current_rt = int(evidence.get("completed_round_trips", 0) or 0)
    if current_rt <= latest_rt:
        raise ValueError("no fresh reconciled Phase-6 round trip exists since the last scale-step proof")
    proof = {
        "scale_step_proof_id": canonical_hash(
            {
                "stage_id": current_stage,
                "evidence_receipt_id": receipt.get("evidence_receipt_id"),
                "completed_round_trips": current_rt,
            }
        ),
        "created_ts": now,
        "stage_id": current_stage,
        ...
        "completed_round_trips": current_rt,
        "new_round_trips": int(evidence.get("new_round_trips", 0) or 0),
        "status": "MINTED",
        "authority_ceiling": "phase7_step_reproof_only",
        "evidence": evidence,
    }
    growth.persist_scale_step_proof(proof)
    return proof
...
def readiness_for_next_stage(canary: CanaryStore, growth: GrowthStore, cfg: Any) -> dict[str, Any]:
    ...
    if receipt.get("authority_ceiling") != "evidence_only":
        reasons.append("phase6_evidence_receipt_authority_invalid")
    return {
        "phase": 7,
...
def build_growth_proposal(
    ...
    *,
    ...
    body = {
        ...
        "evidence_receipt_id": readiness["evidence_receipt"]["evidence_receipt_id"],
        "config_hash": phase7_config_hash(cfg),
    }
    ...
    return GrowthProposal(
        ...
        evidence_hash=readiness["evidence"]["evidence_hash"],
        evidence_receipt_id=str(readiness["evidence_receipt"]["evidence_receipt_id"]),
        config_hash=phase7_config_hash(cfg),
...
def build_growth_approval(
    ...
    *,
    ...
    sealed = seal_phase6_evidence(CanaryStore(growth.data_store), growth, cfg)
    if sealed.get("authority_ceiling") != "evidence_only":
        raise ValueError("Phase-6 evidence receipt authority ceiling is invalid")
    if str(proposal.get("evidence_receipt_id") or "") != str(sealed.get("evidence_receipt_id") or ""):
        growth.reject_active_proposals("approval_safety_recheck_failed:phase6_live_outcomes_changed", now)
        raise ValueError(
            "Phase-7 approval safety recheck detected changed Phase-6 live outcomes; "
            "a fresh proposal is required"
        )
    current_evidence = dict(sealed.get("evidence") or {})
...
def activate_approved_stage(canary: CanaryStore, growth: GrowthStore, cfg: Any, *, actor: str, now_ts: Optional[float] = None, env_interlock: Optional[bool] = None) -> dict[str, Any]:
    ...
    sealed = seal_phase6_evidence(canary, growth, cfg)
    if sealed.get("authority_ceiling") != "evidence_only":
        raise ValueError("Phase-6 evidence receipt authority ceiling is invalid")
    if str(proposal.get("evidence_receipt_id") or "") != str(sealed.get("evidence_receipt_id") or ""):
        raise ValueError("Phase-6 live outcomes changed after approval; a fresh proposal is required")
    current_evidence = dict(sealed.get("evidence") or {})
    safety_reasons = _promotion_gate_reasons(current_evidence, cfg, target)
    if safety_reasons:
...
def evaluate_demotion(canary: CanaryStore, growth: GrowthStore, cfg: Any, *, now_ts: Optional[float] = None) -> dict[str, Any]:
    state = growth.get_state()
    current = int(state.get("current_stage", 0) or 0)
    evidence = dict(seal_phase6_evidence(canary, growth, cfg).get("evidence") or {})
    triggers: list[str] = []
...
def growth_snapshot(canary: CanaryStore, growth: GrowthStore, cfg: Any, *, limit: int = 50) -> dict[str, Any]:
    ...
    return {
        ...
        "scale_step_proofs": growth.scale_step_proofs(limit=limit),
        "incidents": growth.open_incidents()[:limit],
        "audit": growth.audit_rows(limit=limit),
        "phase6_evidence_receipts": canary.evidence_receipts(limit=limit),
        "automatic_promotion": False,
