...
class ResearchCostModel:
    ...
    def __init__(
        ...
        *,
        ...
        configured_ceiling = max(1.0, float(max_total_cost_bps))
        # Keep Phase 2 from self-blocking when verified account-tier fees exceed
        # an older research ceiling. With Kraken's current 80 bps taker fee per
        # side, fees alone are 160 bps, so a 150 bps total ceiling makes every
        # taker-costed forecast impossible before spread/latency/safety.
        minimum_operable_ceiling = (
            (self.taker_fee_bps_per_side * 2.0)
            + self.latency_buffer_bps
            + self.safety_buffer_bps
            + 10.0
        )
        self.max_total_cost_bps = max(configured_ceiling, minimum_operable_ceiling)
