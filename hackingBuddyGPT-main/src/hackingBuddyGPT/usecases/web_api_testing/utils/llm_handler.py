...
class LLMHandler:
    ...
    def execute_prompt(self, prompt: List[Dict[str, Any]]) -> Any:
        ...
        def call_model(prompt: List[Dict[str, Any]]) -> Any:
            ...
            if hasattr(self.llm, 'instructor') and self.llm.instructor:
                ...
                # Fallback for LLMs without instructor (e.g., Ollama)
                # This assumes the LLM can handle the prompt and return a response that can be parsed
                # For Ollama, we need to use /api/chat with tools if supported
                raise NotImplementedError("Instructor not available for this LLM. Use OpenAI-compatible LLMs for full functionality.")
    ...
    def execute_prompt_with_specific_capability(self, prompt: List[Dict[str, Any]], capability: Any) -> Any:
        ...
        def call_model(adjusted_prompt: List[Dict[str, Any]], capability: Any) -> Any:
            ...
            if hasattr(self.llm, 'instructor') and self.llm.instructor:
                ...
                # Fallback for LLMs without instructor
                raise NotImplementedError("Instructor not available for this LLM. Use OpenAI-compatible LLMs for full functionality.")
