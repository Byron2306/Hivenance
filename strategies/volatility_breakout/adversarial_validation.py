...
def _week_key(ts: float) -> str:
    try:
        return datetime.fromtimestamp(float(ts), timezone.utc).strftime("%G-W%V")
    except Exception:
        return "UNKNOWN"
...
def _search_budget_ledger(rows: Sequence[Mapping[str, Any]], candidate_results: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    clean_rows = [row for row in rows if isinstance(row, Mapping)]
    model_ids = {str(row.get("model_id") or "unknown") for row in clean_rows}
    hypotheses = {str(row.get("hypothesis") or "unknown") for row in clean_rows}
    symbols = {str(row.get("symbol") or "unknown") for row in clean_rows}
    policies = {str(row.get("order_policy") or "unknown") for row in clean_rows}
    scenarios = {str(row.get("scenario") or "unknown") for row in clean_rows}
    regimes = {_regime(row) for row in clean_rows}
    direction_set = {str(row.get("direction") or "unknown") for row in clean_rows}
    candidate_keys = {str(row.get("candidate_key") or "unknown") for row in candidate_results if isinstance(row, Mapping)}
    slice_keys = {
        "|".join([
    ...
    return {
        "schema": "phase4_search_budget_ledger_v1",
        "rows_examined": len(clean_rows),
        "candidate_count": len(candidate_results),
        "candidate_keys": len(candidate_keys),
        "model_families_tested": len(model_ids),
        "hypotheses_tested": len(hypotheses),
        "symbols_tested": len(symbols),
        "execution_policies_tested": len(policies),
        "scenarios_tested": len(scenarios),
        "regimes_tested": len(regimes),
        "directions_tested": len(direction_set),
        "slice_keys_tested": len(slice_keys),
        "trial_axes": {
...
class AdversarialValidator:
    ...
    def __init__(self, cfg: Any) -> None:
        ...
        self.distinct_bucket_hours = max(1, int(getattr(cfg, "phase4_distinct_snapshot_bucket_hours", 24) or 24))
        ...
        self.dsr_required = bool(getattr(cfg, "phase4_dsr_required", True))
        ...
        self.max_week_profit_concentration = float(getattr(cfg, "phase4_max_week_profit_concentration", 0.50) or 0.50)
        ...
        self.dsr_min_trial_samples = max(2, int(getattr(cfg, "phase4_dsr_min_trial_samples", 5) or 5))

    def _candidate_scope(self, row: Mapping[str, Any]) -> dict[str, str]:
        mode = str(getattr(self.cfg, "phase4_candidate_key_mode", "model_policy") or "model_policy")
        scope = {
            "model_id": str(row.get("model_id") or "unknown"),
            "order_policy": str(row.get("order_policy") or "unknown"),
        }
        if mode in {"narrow_execution_slice", "model_policy_symbol_direction"}:
            scope["symbol"] = str(row.get("symbol") or "unknown")
            scope["direction"] = str(row.get("direction") or "unknown")
        return scope

    def _candidate_key(self, row: Mapping[str, Any]) -> str:
        scope = self._candidate_scope(row)
    ...
    def validate(self, rows: Sequence[Mapping[str, Any]], *, run_id: str) -> dict[str, Any]:
        ...
        candidate_scopes: dict[str, dict[str, str]] = {}
        for row in normal:
            candidate_key = self._candidate_key(row)
            normal_by_candidate[candidate_key].append(row)
            candidate_scopes.setdefault(candidate_key, self._candidate_scope(row))
        all_trial_sharpes = [
            _trade_sharpe(_finite(row.get("net_return_bps") for row in candidate_rows))
            for candidate_rows in normal_by_candidate.values()
            if len(candidate_rows) >= self.dsr_min_trial_samples
        ]
        ...
        for candidate_key, candidate_normal in sorted(normal_by_candidate.items()):
            scope = candidate_scopes.get(candidate_key) or {}
            model_id = str(scope.get("model_id") or "unknown")
            order_policy = str(scope.get("order_policy") or "unknown")
            if model_id.startswith("baseline_"):
                continue
            candidate_all = [
                row for row in primary
                if all(str(row.get(key) or "unknown") == value for key, value in scope.items())
            ]
            scenarios = {
            ...
            bucket_seconds = self.distinct_bucket_hours * 3600
            distinct_buckets = len({
                int(float(row.get("forecast_ts") or 0.0) // bucket_seconds)
                for row in candidate_normal
                if float(row.get("forecast_ts") or 0.0) > 0
            })
            ...
            week_rows = [dict(row, week=_week_key(float(row.get("forecast_ts") or 0.0))) for row in candidate_normal]
            week_concentration = _positive_profit_concentration(week_rows, "week")
            symbol_concentration_applicable = "symbol" not in candidate_scopes.get(candidate_key, {})
            distinct_months = len({str(row.get("month") or "UNKNOWN") for row in month_rows})
            month_concentration_applicable = distinct_months >= 2
            distinct_weeks = len({str(row.get("week") or "UNKNOWN") for row in week_rows})
            # Substitutes for the month-level check while there isn't enough calendar history for it to
            # apply yet (needs 2+ distinct ISO weeks instead of 2+ distinct calendar months).
            week_concentration_applicable = distinct_weeks >= 2
            walk_positive_ratio = (
            ...
            if distinct_buckets < self.min_distinct_days:
                reasons.append("insufficient_distinct_snapshots")
            ...
            if symbol_concentration_applicable and symbol_concentration > self.max_symbol_profit_concentration:
                reasons.append("symbol_profit_concentration")
            if month_concentration_applicable and month_concentration > self.max_month_profit_concentration:
                reasons.append("month_profit_concentration")
            if week_concentration_applicable and week_concentration > self.max_week_profit_concentration:
                reasons.append("week_profit_concentration")
            if self.dsr_required and dsr.get("dsr_probability", 0.0) < self.dsr_min_probability:
            ...
            recent_metrics = _metrics(candidate_normal[-int(recent_window):])
            robust_score = (
            ...
            result = {
                ...
                "scenarios": scenarios,
                "distinct_time_buckets": distinct_buckets,
                "distinct_snapshot_bucket_hours": self.distinct_bucket_hours,
                ...
                "week_profit_concentration": round(week_concentration, 8),
                "robust_score": round(robust_score, 8),
                "recent_mean_net_bps": recent_metrics.get("mean_net_bps"),
                "recent_samples": recent_metrics.get("samples"),
        ...
        search_budget = _search_budget_ledger(clean, candidate_results)
        return {
            "phase": 4,
            ...
            "search_budget_ledger": search_budget,
