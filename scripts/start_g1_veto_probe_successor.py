...
from strategies.relative_value_lab.g1_research_target import (
    get_latest_g1_research_target,freeze_g1_research_target_successor,
)
...
def main()->int:
    ...
    if predecessor is None:raise RuntimeError("g1_predecessor_campaign_missing")
    previous_target=get_latest_g1_research_target(store)
    target=freeze_g1_research_target_successor(
        store,cfg,model_ids=PROBE_MODELS,order_policy="market",
        predecessor_target_id=(previous_target or {}).get("target_id"),
    )
    prior_policy=policy_from_freeze(predecessor)
    policy=replace(prior_policy,confidence_z=2.690)
    campaign=freeze_g1_campaign_successor(
        store,policy=policy,organs=tuple(predecessor.get("organs") or ()),
        research_target_id=str(target["target_id"]),
        predecessor_campaign_id=str(predecessor["campaign_id"]),
    )
    print(json.dumps({
        "schema":"hivenance_g1_veto_probe_successor_v1",
        "predecessor_campaign_id":predecessor["campaign_id"],
        "predecessor_disposition":"PRIMARY_MODEL_COVERAGE_NULL_15M",
        "successor_campaign":campaign,
        "research_target":target,
        "probe_models":list(PROBE_MODELS),
        "purpose":"TEST_HIVE_VETO_UTILITY_ON_BROAD_PREEXISTING_PROPOSALS",
        "negative_control":"baseline_deterministic_random_v1",
        "multiplicity":{"method":"BONFERRONI","family_size":7,"family_alpha":0.05,"two_sided_confidence_z":2.690},
